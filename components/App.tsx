...
export default function App() {
  ...
  const toggleNodeCompletion = (nodeId: string) => {
    ...
    if (!isCompleted) {
       setNotification({ type: 'achievement', message: `Node Secured: ${activePath.nodes.find(n => n.id === nodeId)?.title} 🏆` });
    }
  ...
  const handlePathActivation = (path: Recommendation) => {
    ...
    const dynamicNodes: RoadmapNode[] = path.milestones.map((m, i) => ({
      ...
      practicePrompt: `Challenge for ${m.title}`,
  ...
  return (
    <div className="flex h-screen overflow-hidden text-slate-900 bg-slate-50 font-sans flex-col md:flex-row">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <div className="md:hidden h-16 bg-white border-b flex items-center justify-between px-6 shrink-0 sticky top-0 z-50">
